import { useNavigate } from 'react-router-dom'
import { Button } from '@/components/ui/button'
import { Card, CardContent } from '@/components/ui/card'
import { getAuth } from './auth'
import nwandoIcon from '@/assets/nwando-icon.png'

/** Google "G" mark, inlined so the page has no external asset dependencies. */
function GoogleG() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A11 11 0 0 0 1 12c0 1.77.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}

export function LoginPage() {
  const navigate = useNavigate()

  const handleSignIn = async () => {
    // MockAuth today; the Google OIDC redirect flow replaces this call when
    // the Python backend is ready (see features/auth/auth.ts).
    await getAuth().signIn()
    navigate('/', { replace: true })
  }

  return (
    <div className="flex min-h-full items-center justify-center bg-gradient-to-br from-brand-50 to-slate-100 p-4">
      <Card className="w-full max-w-md">
        <CardContent className="flex flex-col items-center gap-6 p-10 text-center">
          <img src={nwandoIcon} alt="Nwando Care" className="h-20 w-auto" />
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight">Welcome to Auto-Scheduler</h1>
            <p className="text-sm text-slate-500">
              Nwando Care rota planning and messaging for coordinators
            </p>
          </div>
          <Button variant="secondary" size="lg" className="w-full" onClick={handleSignIn}>
            <GoogleG />
            Sign in with Google
          </Button>
          <p className="text-xs text-slate-400">
            Access is restricted to company Google Workspace accounts.
          </p>
        </CardContent>
      </Card>
    </div>
  )
}
